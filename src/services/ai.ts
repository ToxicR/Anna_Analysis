...
function buildCursorPrompt(
  ...
  const evidencePriority = logText.trim()
    ? `证据优先级：
- 本轮已上传附件，必须以附件为主、代码为辅。
- 多个附件没有主次之分，所有附件共同作为主要证据；不要只看第一个文件，也不要自行降低图片、日志或其他文件的优先级。
- 先逐个读取/观察附件中的直接信息：日志、截图、图片、表格、文本、报错、状态、数值、时间线、用户操作场景等。
- 代码只起辅助作用：用于验证附件线索、解释原因、定位到更准确的文件/方法/接口/状态流转。
- 结论必须优先引用附件中的直接证据；不能脱离附件另起一个纯代码功能分析。
- 如果附件之间或附件与代码线索冲突，明确说明冲突点，并以附件中的实际现象作为当前排查主线。`
    : `证据优先级：
...
硬性要求：
- 有附件时，先分析所有附件，再用代码佐证；不要把代码猜测放在附件事实之前。
- 有附件时，输出中至少包含附件证据和对应代码依据；如果有多个附件，尽量覆盖与问题相关的每个附件。
- 有附件时，代码只用于验证和精准定位问题，不要把回答扩展成脱离附件的代码实现分析。
- 如果找不到对应代码，明确说“附件已命中，但代码侧未定位到对应实现”。
- 没有日志内容时，不要说缺少日志，直接按代码分析。
